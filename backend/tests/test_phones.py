...
def _seed(db, number="+15550001234", **kwargs):
    record = BannedUser(phone_number=number, **kwargs)
    db.add(record)
    db.commit()
    return record
...
def test_ban_phone_permanent(client_admin, db):
    _seed(db)
...
def test_ban_phone_temporary(client_admin, db):
    _seed(db)
...
def test_activate_phone(client_admin, db):
    _seed(db, banned=True, ban_reason="Banned", banned_by="admin")
...
def test_activate_resets_ban_columns(client_admin, db):
    _seed(db, banned=True, ban_reason="Test ban", banned_by="admin")
    r = client_admin.post("/phones/+15550001234/activate", json={
        "reason": "Resolved",
    })
    data = r.json()
...
def test_ban_reason_required(client_admin, db):
    _seed(db)
    r = client_admin.post("/phones/+15550001234/ban", json={
        "is_temporary": False,
        "reason": "",
...
def test_ban_reason_min_length(client_admin, db):
    _seed(db)
    r = client_admin.post("/phones/+15550001234/ban", json={
        "is_temporary": False,
        "reason": "ab",
...
def test_temp_ban_expiry_min_1_hour(client_admin, db):
    _seed(db)
...
def test_helpdesk_cannot_activate(client_admin, client_helpdesk, db):
    _seed(db, banned=True, ban_reason="Admin banned", banned_by="admin")
...
def test_status_filter(client_admin, db):
    _seed(db, number="+15550001111")
    _seed(db, number="+15550002222")
...
def test_search_filter(client_admin, db):
    _seed(db, number="+15550001111")
    _seed(db, number="+15559999999")
